...
if($nbrresult>=1)
	...
	while ($ligne = mysql_fetch_object($requetestats))                                                                              
		...
		$nbvisits[$crawlername]=@$nbvisits[$crawlername] + 1;
		$lastdate[$crawlername]=$date;
		${'listpage'.$crawlername}[] =$page;
		$listpage[]=$page;
		}
	...
	if($daybegin==$daytoday && $monthbegin==$monthtoday && $yearbegin==$yeartoday)
		{
		$testdate=1;
		echo"<h1>".$language['crawler_name']."</h1>\n";
	...
	echo"</th></tr>\n";
	echo"<tr><td class='tableau3'>".$nbrtotcrawlers."</td>\n";	
	...
	elseif($order==1)
		{
		//case pages viewed
		foreach ($listcrawler as $crawlpage)
			{
			${'listpage'.$crawlpage}=array_unique(${'listpage'.$crawlpage});
			$pagecrawler[$crawlpage]=sizeof(${'listpage'.$crawlpage});
			}
		arsort($pagecrawler);
	...
	//change text if more than 100 crawlers	
	if($nbrtotcrawlers>100)
		{
	...
	foreach ($table as $crawl)
		...
		if($comptdata<100)
			{
			${'listpage'.$crawl}=array_unique(${'listpage'.$crawl});
			$nbrpage=sizeof(${'listpage'.$crawl});
			...
			if ($comptligne%2 ==0)
				{
				echo"<tr><td class='tableau3'><a href='index.php?navig=2&amp;period=".$period."&amp;site=".$site."&amp;crawler=".$crawl."'>".$crawldisplay."</a></td>\n";
				...
				{
				echo"<tr><td class='tableau30'><a href='index.php?navig=2&amp;period=".$period."&amp;site=".$site."&amp;crawler=".$crawl."'>".$crawldisplay."</a></td>\n";
