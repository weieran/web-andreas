...
$selection = mysql_select_db($db);
	...
	// we get the actual list number on www.crawltrack.info
	...
	if(function_exists('fopen'))
		{
		$nofile=0;
		$file = fopen("http://www.crawltrack.info/listcrawler/idlist.txt", "r");
	...
	if (!$file OR $nofile==1)
		...
		echo"<h2><a href=\"http://www.crawltrack.info/download/zdn.php?F=crawlerlist.zip\">".$language['download']."</a></h2>\n";		
		...
		if($idlist=="")
			...
			if($idlist==$idlastupdate)
				...
				if( function_exists('fopen'))
					{
					$nofile2=0;
					$file2 = fopen("http://www.crawltrack.info/listcrawler/crawlerlist.txt", "r");
					}
				else
					{
					$nofile2=1;
					...
					if($crawlerlist=="")
						...
						for ($k=1;$k<=$nbrdata;$k++)
							...
							if(in_array($uatest,$listcrawler))
								...
								$requeteinsert = mysql_query($sqlinsert, $connexion);
						...
						$requeteinsertid = mysql_query($sqlinsertid, $connexion);		
						}
					}
				}
			}
